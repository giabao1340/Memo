...
export const sendFriendRequest = async (req, res) => {
    try {
        ...
        const from = req.user._id; // nguoi dang gui yeu cau ket ban

        if (from.toString() === to.toString()) {
            return res.status(400).json({ error: "Bạn không thể gửi yêu cầu kết bạn cho chính mình" });
        }
        ...
        const [alreayFriend, existingRequest] = await Promise.all([
            Friend.findOne({ userA, userB }), // kiem tra 2 nguoi da la ban chua
            FriendRequest.findOne( // kiem tra nguoi khac da gui loi moi ket ban chua 
                {
                    $or: [
                        { from, to },
                        { from: to, to: from }
...
export const declineFriendRequest = async (req, res) => {
    try {
        ...
        if (request.to.toString() !== userId.toString()) {
            return res.status(403).json({ message: "Ban khong co quyenn tu choi loi moi nay" });
        }

        await FriendRequest.findByIdAndDelete(requestId);
        res.status(204).json({ message: "Tu choi loi moi thanh cong" })
...
export const test = async (req, res) => {
    return res.status(204).json({ message: "Test thanh cong" });
}
